    ...
    <div v-else-if="['start','leave'].includes(data.mode)" class="chat grey lighten-5">
      <div class="chat-list pa-3">
        <div v-for="(msg,i) in msgLists" :key="`msg-${i}`">
          <div class="d-flex">
            <v-card :id="`msg-list-${i}`" class="rounded" max-width="70%" :class="msg.is_admin ? 'ml-auto primary lighten-4' : 'success lighten-4'" elevation="0">
          ...
          <p v-if="msg.createdAt" class="grey--text caption mb-3" :class="{'text-right': msg.is_admin}">
